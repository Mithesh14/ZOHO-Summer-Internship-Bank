...
const Dep = () => {
    ...
    React.useEffect(() => {
        fetchAccounts((accounts) => setAccounts(accounts.otherAccounts.filter(e => e.active !== 0)), (message) => toast(message,{position: "top-center", autoClose: 2000,}));
    }, []);
    
    const onSubmitClick = (e) => {    
        if(accountNumber === "") return toast.warning("Select an account Number",{position: "top-center", autoClose: 2000,});
        if(amount === "") return toast.warning("Enter an amount",{position: "top-center", autoClose: 2000,});
        setShowModal(true);
    }
        ...
        <div className={styles.main}>
            <div className={styles.container}>
                <form className={styles.contact_box}>
                    ...
                    <button type="button" onClick={onSubmitClick} className={styles.btn}>Deposit money</button>
                </form>
            </div>
        </div>
        </>
